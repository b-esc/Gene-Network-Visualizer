      ...
      try {
        for (var _iterator4 = params[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
          ...
          if (param.isObjectPattern() || param.isArrayPattern()) {
            ...
            var valueUid = body.scope.generateUidIdentifier("".concat(name, "Type"));
            body.scope.setData("valueUid:".concat(name), valueUid);
            definitions.push(t.variableDeclaration('let', [t.variableDeclarator(valueUid, (0, _convert.default)(context, typeAnnotation))]));
            var _args2 = [t.stringLiteral(name), valueUid];

            if (param.has('optional')) {
              _args2.push(t.booleanLiteral(true));
            }

            invocations.push(t.expressionStatement(context.assert(context.call.apply(context, [methodName].concat(_args2)), t.identifier(name))));
          }
        }
      } catch (err) {
      ...
      if (path.has('returnType')) {
        ...
        if (extra) {
          ...
          if (!path.node.async && promised) {
            ...
            definitions.push(t.variableDeclaration('const', [t.variableDeclarator(returnTypeUid, context.call('return', (0, _convert.default)(context, returnType)))]));
          }
        }
      }
...
function annotationReferencesClassEntity(context, annotation) {
  ...
  try {
    for (var _iterator8 = (0, _typeAnnotationIterator.default)(annotation)[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
